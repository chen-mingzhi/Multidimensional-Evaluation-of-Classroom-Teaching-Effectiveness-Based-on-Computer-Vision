...
def evaluate_student_emo_performance(emotion_labels):
    total_score = 0
    total_students = len(emotion_labels)
...
# ---------------课堂出勤得分
def evaluate_class_chuqin_performance(attended_people, total_people):
    min_score = -2
    max_score = 2

    percentage = attended_people / total_people
...
# ----------------行为识别得分
def calculate_performance(folder_path):
    # 定义得分
    scores = {
        '0': -2,
        '1': -1,
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.txt'):
            ...
            with open(file_path, 'r') as file:
                data = file.readlines()
                total_students += len(data)  # 统计总学生数
...
if __name__ == '__main__':
    ...
    # 计算学生表现综合得分
    emo_average_performance = evaluate_student_emo_performance(emotion_labels)
    print("情绪识别：该堂课的学生表现综合得分为:", emo_average_performance)
    ...
    be_average_performance = calculate_performance(behavior_file_path)
    print("行为识别：该堂课的学生表现质量为:", be_average_performance)
    ...
    chuqin_scr = evaluate_class_chuqin_performance(max_face_count, attend_num)
